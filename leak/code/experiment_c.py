...
def main():
    ...
    ap.add_argument("--out", type=pathlib.Path, default=pathlib.Path("results_c"))
    ...
    # scored against every key. Scores of one text under different keys are only
    # near-independent (repetition inflates variance per text, and that text is shared
    # across keys), so the pooled quantile is unbiased but its effective n is below
    # len(null) * len(keys).
    null_z = np.array([z for r in null for z in r["z_all"]])
    thr = float(np.quantile(null_z, 0.99)) if len(null_z) else 4.0
    # truncated scores get truncated nulls: the null's width comes from repetition,
    # which is length-dependent, so the full-length threshold is wrong for prefixes
    ...
    md += ["## What to compare with Experiment A", "",
           "Hashing found 100% of verbatim leaks of 100 tokens or more of every type at zero observed "
           "false alarms, with no model access; it needs both transcripts instead. The tables above are "
           "what a watermark achieves on matched artifact types with full logit access, only the text "
           "plus the key, and no transformation of the text at all. Scanning against N keys inflates the "
           "watermark's per-text FPR roughly N-fold; hashing's per-pair rate is key-free, but a fleet of "
           "N runs is about N^2 ordered pairs, so expected false alarms grow with fleet size for both.", ""]
