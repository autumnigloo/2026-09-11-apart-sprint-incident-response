...
def main():
    ...
    neg_by_case: dict[str, list[float]] = defaultdict(list)   # for the split-half check
    ...
    # ---- split-half check: fit the threshold on half the cases, score the other half.
    # Everything else in this file is in-sample; this is the held-out sanity number.
    split_rows = []
    if len(by_case) >= 20:
        cids = sorted(by_case)
        for fit_ids, eval_ids in ((cids[0::2], cids[1::2]), (cids[1::2], cids[0::2])):
            fpos = np.array([pos_scores[c] for c in fit_ids])
            fneg = np.array([s for c in fit_ids for s in neg_by_case[c]])
            cand = []
            for t in THRESHOLDS:
                tp = int((fpos >= t).sum())
                fp = int((fneg >= t).sum())
                prec = tp / (tp + fp) if tp + fp else 1.0
                if prec >= args.min_precision and tp:
                    cand.append((tp / len(fpos), -t))
            tfit = -max(cand)[1] if cand else tstar
            epos = np.array([pos_scores[c] for c in eval_ids])
            eneg = np.array([s for c in eval_ids for s in neg_by_case[c]])
            tp = int((epos >= tfit).sum())
            fp = int((eneg >= tfit).sum())
            split_rows.append({"fitted_t": tfit, "recall": round(tp / len(epos), 4),
                               "precision": round(tp / (tp + fp) if tp + fp else 1.0, 4),
                               "fp_hits": fp})
    ...
    for nd in args.negatives:
        # Same convergence reference as the case scoring above. This matters for the
        # same-task dirs (a0, a0_long): a K=2 filter fitted in-batch on eight
        # identical-task runs removes nearly all shared content before scoring and
        # makes the zero self-fulfilling. Falls back to per-dir estimation only when
        # the corpus set was not built.
