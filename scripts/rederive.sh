...
md5 -q code/detector.py code/score.py code/inject.py code/harness.py 2>/dev/null || md5sum code/detector.py code/score.py code/inject.py code/harness.py

log "1. re-plant the 400 cases from the raw corpus and compare with the scored cases"
mkdir -p "$OUT/cases" && cp data/cases/paraphrase_cache.json "$OUT/cases/" 2>/dev/null || true   # cache first, so no new paraphrases are generated
python code/inject.py data/runs/corpus_all --out "$OUT/cases" --reps 5 --seed 0 >/dev/null 2>"$OUT/inject.err"
python3 - <<'PY'
import json
a=[json.loads(l) for l in open("data/cases/labels.jsonl")]
b=[json.loads(l) for l in open("data/rederived/cases/labels.jsonl")]
...
S="data/cases"
python code/score.py $S --fixed 0.35 --negatives data/runs/corpus_all data/runs/a0 data/runs/a0_long --out $OUT/main >/dev/null 2>&1
python code/score.py $S --fixed 0.35 --score jaccard --out $OUT/jaccard >/dev/null 2>&1
python code/score.py $S --fixed 0.35 --no-origination --out $OUT/no_origination >/dev/null 2>&1
python code/score.py $S --fixed 0.35 --common-runs 0 --out $OUT/no_convergence >/dev/null 2>&1
...
for d in corpus_all a0 a0_long swarm_unspecified swarm_prohibited swarm_directed swarm_unspecified_gpt41 swarm_prohibited_gpt41 swarm_unspecified_gpt41_impossible; do
  ...
  n=$(python code/detector.py data/runs/$d --threshold 0.35 2>/dev/null | grep -o "^[0-9]* hits" | head -1)
  echo "$d: $n at 0.35 ($(python code/shared_usage.py data/runs/$d 2>/dev/null | tail -1 | grep -o '[0-9]* commands' || echo 'n/a'))"
done
for d in swarm_directed_explicit swarm_directed_explicit_2; do
  [[ -d data/runs/$d ]] || continue
  python code/detector.py data/runs/$d --threshold 0.35 --out $OUT/$d.json >/dev/null 2>&1
  python code/detector.py data/runs/$d --threshold 0.35 --no-origination --out $OUT/${d}_noorig.json >/dev/null 2>&1
  echo "$d: $(python code/verify_edges.py $OUT/$d.json | tail -1)"
  echo "$d without origination: $(python code/verify_edges.py $OUT/${d}_noorig.json | tail -1)"
  python code/topology.py $OUT/$d.json | python3 -c "import json,sys;d=json.load(sys.stdin);print('   label',d['label'],'broadcasters',d['broadcasters'])"
done
